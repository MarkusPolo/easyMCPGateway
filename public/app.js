...
async function connectCodexOAuth() {
    try {
        ...
        const codeText = data.user_code ? `\n\nOne-time code: ${data.user_code}` : '';
        alert(`Sign in in the opened browser window at ${data.verification_uri || 'auth.openai.com/codex/device'}.${codeText}\n\nAfter you complete login, click OK to refresh status.`);
