...
export function startAdminServer(toolManager: ToolManager, port: number = 8080) {
    ...
    app.get('/oauth/codex/callback', async (req, res) => {
        return res.status(410).send('Callback-based OAuth is disabled. Use Codex SDK device login (/api/llm/codex/oauth/start) with auth.openai.com/codex/device and the one-time code.');
    });
    ...
    app.post('/api/llm/codex/oauth/start', requireRoles(['supervisor']), async (req, res) => {
        try {
            const device = await codexOAuthService.startDeviceAuth();
            res.json({
                mode: device.mode,
            ...
            res.status(500).json({ error: error.message || 'Failed to start Codex SDK auth flow' });
        }
    });

    app.post('/api/llm/codex/oauth/manual', requireRoles(['supervisor']), async (req, res) => {
        return res.status(410).json({
            error: 'Manual token injection is no longer supported. Use Codex SDK device auth via /api/llm/codex/oauth/start.'
        });
    });
